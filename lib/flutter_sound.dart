...
class FlutterSound {
  ...
  Future<String> pauseRecorder() async {
    if (_audioState == t_AUDIO_STATE.RECORDING_PAUSED) {
      throw new RecorderNotRunningException('Recorder is currently paused');
    }

    if (_audioState != t_AUDIO_STATE.IS_RECORDING) {
      throw new RecorderNotRunningException('Recorder is not recording');
    }

    String result = await _channel.invokeMethod('pauseRecorder');
    _audioState = t_AUDIO_STATE.RECORDING_PAUSED;
  ...
  Future<String> resumeRecorder() async {
    if (_audioState != t_AUDIO_STATE.RECORDING_PAUSED) {
      throw new RecorderNotRunningException('Recorder is not currently paused');
    }
...
class RecorderNotRunningException implements Exception {
  final String message;
  RecorderNotRunningException(this.message);
}
