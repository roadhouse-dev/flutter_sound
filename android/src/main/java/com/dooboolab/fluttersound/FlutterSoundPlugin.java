...
public class FlutterSoundPlugin implements MethodCallHandler, PluginRegistry.RequestPermissionsResultListener, AudioInterface {
    ...
    public void startRecorder(Integer numChannels, Integer sampleRate, Integer bitRate, t_CODEC codec, int androidEncoder, int androidAudioSource, int androidOutputFormat, String path, final Result result) {
        ...
        model.setRecorderStatus(AudioModel.RECORDER_STATUS_RECORDING);
        ...
        try {
            ...
            this.model.setLastRecorderTick(SystemClock.elapsedRealtime());
            this.model.setRecorderTicker(() -> {

                final long currentTime = SystemClock.elapsedRealtime();
                if (this.model.getRecorderStatus() == AudioModel.RECORDER_STATUS_RECORDING) {
                    final long durationDelta = currentTime - this.model.getLastRecorderTick();
                    this.model.setRecordTime(this.model.getRecordTime() + durationDelta);
                }

                this.model.setLastRecorderTick(currentTime);

                try {
                    JSONObject json = new JSONObject();
                    json.put("current_position", String.valueOf(this.model.getRecordTime()));
    ...
    public void pauseRecorder(final Result result) {
        ...
        if (Build.VERSION.SDK_INT >= sdkCompat.VERSION_CODES_M) {
            model.setRecorderStatus(AudioModel.RECORDER_STATUS_PAUSED);
    ...
    public void stopRecorder(final Result result) {
        ...
        model.setRecorderStatus(AudioModel.RECORDER_STATUS_STOPPED);
